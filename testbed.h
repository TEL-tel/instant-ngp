...
#include<neural-graphics-primitives/threshold.h>//edit
...
public:
	...
	int marching_cubes(ivec3 res3d, const BoundingBox& render_aabb, const mat3& render_aabb_to_local, float thresh, std::vector<ThreshBox> other_threshs);//edit
	...
	void compute_and_save_marching_cubes_mesh(const fs::path& filename, ivec3 res3d = ivec3(128), BoundingBox aabb = {}, float thresh = 2.5f, bool unwrap_it = false, std::vector<ThreshBox> other_threshs={});//edit
	...
	ERenderMode m_render_mode = ERenderMode::Density;
	...
	struct ThreshBox{
		...
		ThreshBox(BoundingBox aabb, float thresh){};
	};
	*/
	int thresh_i=0;
	BoundingBox threshBox;
	std::vector<ThreshBox> threshBoxes;
